...
pub fn TtyInterface(comptime OutputWriter: type) type {
    return struct {
        ...
        fn updateSearch(self: *Self) !void {
        ...
        fn drawMatch(self: *Self, choice: String, selected: bool) void {
            const tty = self.tty;
            const options = self.options;
        ...
        const Action = struct {
            fn exit(tty_interface: *Self) !void {
                tty_interface.clear();
                tty_interface.exit = 1;
            }

            fn delChar(tty_interface: *Self) !void {
            ...
            fn delCharOrExit(tty_interface: *Self) !void {
            ...
            fn delWord(tty_interface: *Self) !void {
            ...
            fn delAll(tty_interface: *Self) !void {
                tty_interface.search.len = 0;
                tty_interface.cursor = 0;
            }

            fn emit(tty_interface: *Self) !void {
                ...
                if (choices.selections.count() == 0) {
                    if (choices.getResult(choices.selection)) |selection| {
                        try tty_interface.output_writer.print("{s}\n", .{choices.getString(selection.str)});
                    } else {
            ...
            fn next(tty_interface: *Self) !void {
                tty_interface.choices.next();
            }

            fn prev(tty_interface: *Self) !void {
                tty_interface.choices.prev();
            }

            fn beginning(tty_interface: *Self) !void {
                tty_interface.cursor = 0;
            }

            fn end(tty_interface: *Self) !void {
                tty_interface.cursor = tty_interface.search.len;
            }

            fn left(tty_interface: *Self) !void {
                if (tty_interface.cursor > 0) {
                    tty_interface.cursor -= 1;
                }
            }

            fn right(tty_interface: *Self) !void {
                if (tty_interface.cursor < tty_interface.search.len) {
                    tty_interface.cursor += 1;
                }
            }

            fn pageUp(tty_interface: *Self) !void {
                try tty_interface.update();
            ...
            fn pageDown(tty_interface: *Self) !void {
                try tty_interface.update();
                var choices = tty_interface.choices;
                const available = choices.numResults();
            ...
            fn select(tty_interface: *Self) !void {
                try tty_interface.update();
                const choices = tty_interface.choices;
                if (choices.getResult(choices.selection)) |selection| {
            ...
            fn ignore(_: *Self) !void {}
        };

        const KeyBinding = struct {
            key: []const u8,
            action: *const fn (tty_interface: *Self) anyerror!void,
        };
        ...
        fn handleInput(self: *Self, s: []const u8, handle_ambiguous_key: bool) !void {
